export function timeRequiredToBuy(
  clientRequiredTickets: number[],
  targetClientIndex: number
): number {
  const clients = clientRequiredTickets.map((ticketsCount, index) => ({
    clientIndex: index,
    ticketsCount,
  }));

  const queue = new Queue<(typeof clients)[number]>();
  clients.forEach((client) => queue.enqueue(client));
  ...
  while (!queue.empty()) {
    ...
    if (client && client.ticketsCount > 1) {
      queue.enqueue({ ...client, ticketsCount: client.ticketsCount - 1 });
    } else if (client?.clientIndex === targetClientIndex) {
      break;
    }
  }
